...
def test_equivariance_t1(_equivariance_test_utils):
    import sake
    from sake.utils import assert_almost_equal_tensor
    h0, x0, translation, rotation, reflection = _equivariance_test_utils
    x0 = x0.unsqueeze(0).repeat(2, 1, 1)
    layer = sake.RecurrentDenseSAKELayer(7, 6, 5, residual=False, update_coordinate=True, order=1)
...
def test_equivariance_t1_with_batch(_equivariance_test_utils):
    import sake
    from sake.utils import assert_almost_equal_tensor
    h0, x0, translation, rotation, reflection = _equivariance_test_utils
    x0 = x0.unsqueeze(0).unsqueeze(0).repeat(3, 2, 1, 1)
    h0 = h0.unsqueeze(0).repeat(3, 1, 1)
    layer = sake.RecurrentDenseSAKELayer(7, 6, 5, residual=False, update_coordinate=True, order=1)
