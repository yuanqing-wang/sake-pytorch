...
def test_simple_forward():
    ...
    x = torch.randn(8, 3)
    h = torch.randn(8, 5)
    layer = sake.DenseSAKELayer(5, 6, 7, distance_filter=sake.RBF)
    _x, _h = layer(h, x)
...
def test_layer_simple_graph_equivariant():
    ...
    layer = sake.DenseSAKELayer(in_features=7, hidden_features=8, out_features=9, distance_filter=sake.RBF)
