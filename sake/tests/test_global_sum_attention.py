...
def test_global_sum_simple_graph_invariant():
    ...
    layer = sake.GlobalSumSAKELayer(in_features=7, hidden_features=8, out_features=9)

    # original
    h_original = layer(g, h0, x0)
...
def test_layer_simple_graph_equivariant_with_attention():
    import torch
    import dgl
    import sake
    g = dgl.rand_graph(5, 8)
    ...
    net = sake.EGNN(
        ...
        sum_readout=torch.nn.Linear(9, 9),
        global_attention=sake.GlobalSumSAKELayer(
            in_features=9,
            hidden_features=10,
            out_features=9,
        )
